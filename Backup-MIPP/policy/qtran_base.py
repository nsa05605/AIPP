...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 17 16:00:06 2020
...
from network.qtran_net import Qtran_basenet
...
class QtranBase:
    def __init__(self, args):
        ...
        self.eval_joint_q = Qtran_basenet(args)
        self.target_joint_q = Qtran_basenet(args)
        ...
        self.eval_parameters = list(self.eval_joint_q.parameters()) + eval_net_parameters
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon = None,agent_id=None): 
        ...
        joint_q_evals, joint_q_targets = self.get_qtran(batch, hidden_evals, hidden_targets, opt_onehot_target)
    ...
    def get_qtran(self, batch, hidden_evals, hidden_targets, local_opt_actions, hat = False):
        ...
        if hat:
            q_evals = self.eval_joint_q(hidden_evals, local_opt_actions)
            q_targets = None
            q_evals = q_evals.reshape(episode_num, -1, 1).squeeze(-1)
            
        else:
            q_evals = self.eval_joint_q(hidden_evals, u_onehot)
            q_targets = self.target_joint_q(hidden_targets, local_opt_actions)
            q_evals = q_evals.reshape(episode_num, -1, 1).squeeze(-1)
            q_targets = q_targets.reshape(episode_num, -1, 1).squeeze(-1)
